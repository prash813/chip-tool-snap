...
void PairingCommand::LogTheCommisioningAttempt(bool iscommisioningsuccess)
{
	...
	if(snpdatapath != NULL){
		...
		snprintf(buffer, 255, "%s/%lu_pairres.json", snpdatapath, (unsigned long)mNodeId);
		ChipLogProgress(chipTool, "Device commissioning Result..path %s\n", buffer);
		int fd = open(buffer,  O_RDWR|O_CREAT|O_TRUNC, 0660);

		if(fd != -1){

			if(iscommisioningsuccess)
				snprintf(databuffer, 255, "{\n\t\"Results\": [\n\t\t{\n\n\t\t\t\"Pairing\": %s,\n\t\t\t\"nodeid\": \"%lu\"\n\n\t\t}]\n\n\n}\n", "\"Success\"", (unsigned long)mNodeId);
			else
				snprintf(databuffer, 255, "{\n\t\"Results\": [\n\t\t{\n\n\t\t\t\"Pairing\": %s\n\n\t\t}]\n\n\n}\n", "\"Failure\"");	
