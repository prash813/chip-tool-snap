...
//prash@canonical
void Commands::LogTheCmdFailure(const char* clstrname, const char*cmdname)
{

	char*snpdatapath = getenv("SNAP_DATA");

	if(snpdatapath != NULL){
		char buffer[256]={0,};
		char databuffer[256]={0,};
		snprintf(buffer, 255, "%s/%s_%s.json",snpdatapath, clstrname, cmdname);
		ChipLogProgress(chipTool, "device cluster %s cmds %s result..path %s\n", clstrname, cmdname, buffer);
		int fd = open(buffer,  O_RDWR|O_CREAT|O_TRUNC, 0660);

		if(fd != -1){

			snprintf(databuffer, 255, "{\n\t\"Results\": [\n\t\t{\n\n\t\t\t\"CmdRes\": \"Failure\",\n\t\t\t\"Command\": \"%s\"\n\n\t\t}]\n\n\n}\n",  cmdname);
			write(fd, databuffer, strlen(databuffer));
			close(fd);
		}
	}
...
int Commands::Run(int argc, char ** argv)
{
	...
	if(err != CHIP_NO_ERROR){
		...
		ChipLogError(chipTool, "Run command failed for cluster %s\n", argv[1]);
		if(!clustername.compare("onoff")){
			LogTheCmdFailure(argv[1], argv[2]);
		}		
	}
