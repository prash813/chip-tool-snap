...
CHIP_ERROR ExtractIssuerNameFromX509Cert(const ByteSpan & certificate, std::string& outissuername)
{
	ASN1_OBJECT * commonNameObj = OBJ_txt2obj("2.5.4.3", 1);
	CHIP_ERROR err                     = CHIP_NO_ERROR;
	X509 * x509Certificate             = nullptr;
	...
	x509Certificate = d2i_X509(nullptr, &pCertificate, static_cast<long>(certificate.size()));
	VerifyOrReturnError(x509Certificate != nullptr, err = CHIP_ERROR_NO_MEMORY);

	issuername = X509_get_issuer_name(x509Certificate); 
	VerifyOrReturnError(issuername != nullptr, err = CHIP_ERROR_INTERNAL);

	for (x509EntryCountIdx = 0; x509EntryCountIdx < X509_NAME_entry_count(issuername); ++x509EntryCountIdx)
	{
		X509_NAME_ENTRY * name_entry = X509_NAME_get_entry(issuername, x509EntryCountIdx);
		...
		if (attrType == DNAttrType::kCommonName)
		{
			ASN1_STRING * data_entry = X509_NAME_ENTRY_get_data(name_entry);
			...
			int len = ASN1_STRING_length(data_entry);
			outissuername.append((const char*)str, len);
		}
	}
